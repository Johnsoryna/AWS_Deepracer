...
def reward_function(params):
    ...
    def closest_2_racing_points_index(racing_coords, car_coords):
        distances = [
            dist_2_points(x, car_coords[0], y, car_coords[1])
    ...
    ratio = speed / (optimal_speed + 1e-9)

    if ratio > 1.0:
        ratio = 1.0
    speed_factor = ratio**2  

    optimal_heading = closest_coords[4]
    heading_diff = abs(optimal_heading - heading)
    heading_factor = math.exp(-0.1 * (heading_diff**2))

    optimal_acc = closest_coords[3]
    acc_diff = abs(optimal_acc - acceleration)
    acceleration_factor = math.exp(-(acc_diff**2) / 4.0)

    reward = 1.0
    reward *= distance_factor
    reward *= speed_factor
    reward *= heading_factor
    reward *= acceleration_factor
