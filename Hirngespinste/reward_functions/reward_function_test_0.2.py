...
class Reward:
    ...
    def reward_function(self, params):
        ...
        speed_reward = (speed - MIN_SPEED) / (MAX_SPEED - MIN_SPEED) if speed >= MIN_SPEED else 0
        reward += speed_reward * 6  
        ...
        if abs(steering_angle) > 22:
            reward *= 0.8 
        if abs(steering_angle) > 10 and speed >= (MAX_SPEED * 0.75):
            reward += 2 

        progress_reward = max(1, 15 * (progress / steps))
        reward += progress_reward

        if progress == 100:
            bonus = max(10, 150 - (steps * 0.75)) 
            reward += bonus
